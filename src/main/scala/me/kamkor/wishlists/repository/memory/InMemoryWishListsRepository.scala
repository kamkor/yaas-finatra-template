...
class InMemoryWishListsRepository extends WishListsRepository {
  ...
  override def get(tenant: String, id: String): Future[Option[WishList]] = Future {
    storage.get(WishListKey(tenant, id))
  }

  override def delete(tenant: String, id: String): Future[Unit] = Future {
    storage -= WishListKey(tenant, id)
  }

  override def update(tenant: String, wishList: WishList): Future[WishList] = Future {
